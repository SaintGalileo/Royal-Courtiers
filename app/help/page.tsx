...
const faqs = [
  {
    ...
    answer: "The Family Love Built; Bearers of the Seal of the Universal Monarch"
  },
];
...
export default function HelpPage() {
  ...
  return (
    <main className="min-h-screen w-full bg-background text-foreground flex flex-col pt-24 font-[family-name:var(--font-libre)]">
      ...
      <div className="flex-1 px-4 py-16 md:py-24 max-w-6xl mx-auto w-full">
        {/* Hero Section */}
        <div className="text-center mb-20 animate-in fade-in slide-in-from-bottom-8 duration-700">
          ...
          <p className="text-lg text-zinc-600 dark:text-zinc-400 max-w-2xl mx-auto">
            Find answers to common questions or reach out to our event directorate directly.
          </p>
        </div>
          ...
          <div className="space-y-4 max-w-4xl mx-auto">
            {faqs.map((faq, idx) => (
              ...
              >
                ...
                >
                  <span className="font-semibold text-zinc-800 dark:text-zinc-200">{faq.question}</span>
                  <ChevronDown className={`w-5 h-5 text-zinc-400 transition-transform duration-300 ${openFaq === idx ? "rotate-180" : ""}`} />
                </button>
                <div
        ...
        <section className="animate-in fade-in slide-in-from-bottom-8 duration-700 delay-400">
          <div className="flex items-center gap-3 mb-10 pb-4 border-b border-zinc-100 dark:border-zinc-800">
            ...
            <h2 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100 uppercase tracking-widest">
              Contact the <span className="text-(--primary-gold)">Directorate</span>
            </h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {contactExcos.map((exco) => (
              <div key={exco.role} className="group relative bg-white dark:bg-zinc-900/50 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 flex flex-col items-center text-center transition-all hover:-translate-y-2 hover:shadow-xl hover:border-(--primary-gold)/30">
                <div className="relative w-32 h-32 mb-6 rounded-full overflow-hidden border-4 border-(--primary-gold)/20 group-hover:border-(--primary-gold)/40 transition-colors">
                  {exco.img ? (
                    <img src={exco.img} alt={exco.name} className="w-full h-full object-cover" />
                  ) : (
                ...
                <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-100 mb-1">{exco.name}</h3>
                <p className="text-xs font-bold tracking-widest text-(--primary-gold) uppercase mb-6">{exco.role}</p>

                <div className="w-full pt-6 border-t border-zinc-100 dark:border-zinc-800 space-y-3">
                  <a href={`tel:${exco.phone}`} className="flex items-center justify-center gap-2 text-sm text-zinc-500 dark:text-zinc-400 hover:text-(--primary-gold) transition-colors">
